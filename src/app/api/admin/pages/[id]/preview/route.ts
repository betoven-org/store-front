...
<body>
  <div class="preview-bar">Preview — Live</div>
  <div id="preview-root">${renderedSections}</div>
  <script>
  // ── Live Preview: listen for section updates from parent editor ──
  (function() {
    ...
    window.addEventListener("message", function(event) {
      ...
      if (msg.type === "brasa:sections-update" && Array.isArray(msg.blocks)) {
        // Re-render sections client-side
        var html = msg.blocks.map(function(s, i) {
          return renderSectionClient(s, i);
        }).join("");
        root.innerHTML = html;
      }
    });
    ...
    function renderSectionClient(s, index) {
      var p = s.props || {};
      var component = s.component || s.type || "unknown";
      var isEven = index % 2 === 0;

      // Campaign sections (type-based)
      if (s.type && !s.component) {
        switch (s.type) {
          case "hero":
            return '<section class="lp-hero"><div class="lp-hero-inner">'
              + (s.eyebrow ? '<span class="lp-eyebrow">' + esc(s.eyebrow) + '</span>' : '')
              + '<h1 class="lp-hero-heading">' + esc(s.heading) + '</h1>'
              + '<p class="lp-hero-desc">' + esc(s.description) + '</p>'
              + '</div><div class="lp-hero-gradient"></div></section>';
          case "trust_badges":
            return '<section class="lp-trust-badges"><div class="lp-trust-inner">'
              + (s.items||[]).map(function(item) {
                return '<div class="lp-trust-item">'
                  + (item.icon ? '<img src="'+esc(item.icon)+'" class="lp-trust-icon" width="36" height="36"/>' : '')
                  + '<span class="lp-trust-text">'+esc(item.text)+'</span></div>';
              }).join('') + '</div></section>';
          case "image_text":
            return '<section class="lp-content-block '+(isEven?'lp-bg-white':'lp-bg-alt')+'"><div class="lp-content-inner lp-layout-img-left">'
              + '<div class="lp-content-media">'+(s.image&&s.image.src?'<img src="'+esc(s.image.src)+'" class="lp-content-img"/>':'<div class="lp-img-placeholder"></div>')+'</div>'
              + '<div class="lp-content-text"><h2 class="lp-content-heading">'+esc(s.heading)+'</h2><p class="lp-content-body">'+esc(s.text)+'</p></div>'
              + '</div></section>';
          case "text_image":
            return '<section class="lp-content-block '+(isEven?'lp-bg-white':'lp-bg-alt')+'"><div class="lp-content-inner lp-layout-img-right">'
              + '<div class="lp-content-text"><h2 class="lp-content-heading">'+esc(s.heading)+'</h2><p class="lp-content-body">'+esc(s.text)+'</p></div>'
              + '<div class="lp-content-media">'+(s.image&&s.image.src?'<img src="'+esc(s.image.src)+'" class="lp-content-img"/>':'<div class="lp-img-placeholder"></div>')+'</div>'
              + '</div></section>';
          case "benefits_icons":
            return '<section class="lp-benefits"><div class="lp-benefits-inner">'
              + (s.items||[]).map(function(item) {
                return '<div class="lp-benefit-card">'
                  + (item.icon ? '<img src="'+esc(item.icon)+'" class="lp-benefit-icon" width="56" height="56"/>' : '<div class="lp-benefit-icon-placeholder"></div>')
                  + '<p class="lp-benefit-text">'+esc(item.text)+'</p></div>';
              }).join('') + '</div></section>';
          case "product_shelf":
            return '<section class="lp-products"><div class="lp-products-inner">'
              + (s.heading?'<h2 class="lp-products-heading">'+esc(s.heading)+'</h2>':'')
              + '<div class="lp-products-grid">'
              + (s.products||[]).map(function(prod) {
                return '<a href="'+esc(prod.href||'#')+'" class="lp-product-card" target="_blank">'
                  + (prod.image?'<img src="'+esc(prod.image)+'" class="lp-product-img"/>':'<div class="lp-product-img-placeholder"></div>')
                  + '<div class="lp-product-info"><h3 class="lp-product-name">'+esc(prod.name)+'</h3>'
                  + (prod.price?'<span class="lp-product-price">'+esc(prod.price)+'</span>':'')
                  + '<span class="lp-product-cta">Comprar agora →</span></div></a>';
              }).join('') + '</div></div></section>';
          default:
            return '<section class="section-unknown"><div class="unknown-header">'+(index+1)+' '+esc(s.type)+'</div></section>';
        }
      }

      // CMS sections (component-based)
      switch (component) {
        case "Banner":
          return '<section class="section-banner" style="background:'+ esc(p.backgroundColor||'#f3f4f6') +';min-height:'+(p.height==='grande'?'320px':p.height==='pequeno'?'160px':'240px')+'">'
            + (p.image?'<img src="'+esc(p.image)+'" alt="'+esc(p.alt||'')+'" class="banner-img"/>':'')
            + '<div class="banner-overlay">'+(p.title?'<h2 class="banner-title">'+esc(p.title)+'</h2>':'')+(p.description?'<p class="banner-desc">'+esc(p.description)+'</p>':'')+'</div></section>';
        case "Hero":
          return '<section class="section-hero'+(p.dark?' hero-dark':'')+'"'+(p.backgroundImage?' style="background-image:url('+esc(p.backgroundImage)+')"':'')+'>'
            + '<div class="hero-content"><h1 class="hero-title">'+esc(p.title||'')+'</h1>'
            + (p.subtitle?'<p class="hero-subtitle">'+esc(p.subtitle)+'</p>':'')
            + (p.cta?'<a href="'+esc(p.cta.href||'#')+'" class="hero-cta">'+esc(p.cta.label||'CTA')+'</a>':'')
            + '</div></section>';
        case "LpHero":
          return '<section class="section-lphero"><div class="lphero-content">'
            + '<h1 class="lphero-headline">'+esc(p.headline||'')+'</h1>'
            + (p.subheadline?'<p class="lphero-sub">'+esc(p.subheadline)+'</p>':'')
            + '</div></section>';
        case "Features":
          return '<section class="section-features"><h2>'+esc(p.title||'')+'</h2>'
            + '<div class="features-grid cols-'+(p.columns||'3')+'">'
            + (p.items||[]).map(function(item) {
              return '<div class="feature-card">'+(item.icon?'<img src="'+esc(item.icon)+'" class="feature-icon" alt=""/>':'')
                + '<h4>'+esc(item.title||'')+'</h4><p>'+esc(item.description||'')+'</p></div>';
            }).join('')+'</div></section>';
        case "ImageText":
          return '<section class="section-imagetext '+(p.imagePosition==='direita'?'img-right':'img-left')+'">'
            + (p.image?'<div class="it-image"><img src="'+esc(p.image)+'" alt="'+esc(p.imageAlt||'')+'"/></div>':'')
            + '<div class="it-content"><h2>'+esc(p.title||'')+'</h2><div class="it-text">'+(p.text||'')+'</div></div></section>';
        default:
          return '<section class="section-unknown"><div class="unknown-header"><span class="section-badge">'+(index+1)+'</span> '+esc(component)+'</div>'
            + '<div class="unknown-props">'+Object.keys(p).map(function(k){return '<span class="prop-pill"><b>'+esc(k)+'</b></span>';}).join('')+'</div></section>';
      }
    }
  })();
  </script>
